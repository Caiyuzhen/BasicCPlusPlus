...
int main() {
	...
	// ⚡️ 自动的类型转化 ————————————————————————————————————————————————————————————————————————————
	// 1. 整型赋值给 bool
	bool abc = 25; // 非 0 的值赋值给 bool , 则值都为 1, true     ；   如果赋值为 0 , 则都为 false

	// 2. bool 赋值给整型
	short def = false;  // 会把 false 转化为 0 然后赋值给 def， 所以打印出来是 0, 因为底层都是 0 1

	// 3. 浮点数赋值给整型
	int gh = 3.14; // 会去除浮点小数部份

	// 4. 整型赋值给浮点类型
	float jkl = 3; // 会加上浮点的小数, 打印出来为 3.0

	// 5. 赋值超出了整型范围
	unsigned short mn = 65537; // 会回退为【最小值 0】
}
